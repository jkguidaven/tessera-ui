...
All interactive elements show a visible focus indicator on `:focus-visible`. The focus ring is a 3px `box-shadow` using `--ts-color-focus-ring` (primary orange at 40% opacity). A danger variant (`--ts-color-focus-ring-danger`) is used for error-state inputs.

```css
/* Applied automatically by all Tessera UI components */
:focus-visible {
  outline: none;
  box-shadow: var(--ts-focus-ring); /* 0 0 0 3px rgba(228,92,14, 0.4) */
}
```
